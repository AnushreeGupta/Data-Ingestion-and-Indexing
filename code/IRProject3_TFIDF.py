...
with codecs.open('/home/anushree/Desktop/IRProject3/queries.txt', encoding='utf-8') as f:
    for line in f:
        ...
        inurl = 'http://localhost:8983/solr/TFIDF/select?defType=edismax&fl=id,score&indent=on&q='+query+'&qs=70&rows=200&tie=0.1&wt=json'
        qid = count
        IRModel = 'TFIDF'
        ...
        for doc in docs:
            if count < 10:
                outf.write('00'+str(qid) + ' ' + 'Q' + str(count) + ' ' + str(doc['id']) + ' ' + str(rank) + ' ' + str(doc['score']) + ' ' + 'VSM' + '\n')
            else:
                outf.write('0'+str(qid) + ' ' + 'Q' + str(count) + ' ' + str(doc['id']) + ' ' + str(rank) + ' ' + str(doc['score']) + ' ' + 'VSM' + '\n')
            rank += 1
        outf.close()
        count += 1
